...
# Skip all tests if SCYLLA_HOST is not set or integration tests are disabled
...
class TestCompression:
    ...
    def test_compression_with_large_item(
        self, table_name: str, skip_if_scylla_version_below: Any
    ) -> None:
...
class TestTLSConfiguration:
    ...
    @pytest.mark.skip(reason="Requires HTTPS-enabled Scylla cluster")
    def test_tls_system_default(self) -> None:
        """Test TLS with system CA certificates."""
    ...
    @pytest.mark.skip(reason="Requires HTTPS-enabled Scylla cluster with custom CA")
    def test_tls_custom_ca(self) -> None:
        """Test TLS with custom CA certificate."""
        from pathlib import Path

        from alternator import TlsConfig

        # This test requires a real CA certificate file
        ca_path = Path("/path/to/ca.pem")
        if not ca_path.exists():
            pytest.skip("CA certificate not found")
