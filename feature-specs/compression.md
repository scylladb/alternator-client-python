...
The single encoding token must be matched case-insensitively after trimming optional whitespace
from its coding name and ignoring parameters beginning with the first semicolon. These
normalization steps do not make multiple header fields or comma-separated content-coding tokens
eligible for decoding.
...
even when its body is compressed. Failure before receiving final response headers, including a
transport-level request-body failure, follows [Node health](node-health.md). A decompression failure
after receiving final response headers must not be mistaken for absence of a final HTTP response.
...
- A compressed response with HTTP `500`, `502`, `503`, or `504` is decoded for error handling,
  while its status remains health-neutral.
- A decompression error after final response headers arrive must clear transmission bookkeeping
  without being double-counted as a transport failure.
...
deflate responses must be decoded transparently after the required case and parameter normalization,
with stale encoding metadata removed.
