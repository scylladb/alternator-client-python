...
Matching reusable leases must share the current physical cluster and must receive independent
...
A private lease must be exclusive and must expose cluster start, cluster stop, node start, node
...
Before launching a child, the harness must create and open a durable per-command output file and
configure the child to redirect stdout and stderr directly to it, so output is persisted as it is
produced and survives process termination. Before destructive cluster removal, the harness must
snapshot command output, configuration, and node logs without copying private keys. If the snapshot
cannot be transferred to the external diagnostics directory, the live run must be retained for a
later attempt.
