...
| Final HTTP response | Non-informational response; receiving its headers completes attempt attribution even if later body handling fails. |
...
non-negative bits as `Int63`. `Int31` is the high 31 bits of `Int63`.

An `Intn` bound must satisfy `1 <= n <= 2^63 - 1`; other bounds are invalid. When
`n <= 2^31 - 1`, set width `w = 31` and draw `value = Int31`. Otherwise set `w = 63` and draw
`value = Int63`. When `n` is a power of two, return `value & (n - 1)`. Otherwise set
`limit = 2^w - 1 - (2^w mod n)`, redraw values of the same width while `value > limit`, and return
the accepted value modulo `n`.
...
Receiving final HTTP response headers clears the in-flight endpoint; receiving informational
response headers does not. A failure before receiving final response headers is attributed to that
...
- An HTTP `500`, `502`, `503`, or `504` response clears in-flight accounting without creating a
  health observation.
- A transport exception before final response headers remains associated with the selected endpoint.
...
- A transport invocation not associated with a logical query is outside this routing contract and
  may be delegated unchanged. If request-scoped routing state cannot be found while preparing a
  transmission for a logical query, routing must fail locally before sending; transport must neither
  use the request's existing endpoint nor attach the transmission to another query's plan.
