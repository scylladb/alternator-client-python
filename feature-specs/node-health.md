...
Node health protects requests from endpoints that repeatedly fail before returning a final HTTP
...
| Failure before a final HTTP response | `TRAFFIC_FAILURE` |

Application, validation, authentication, conditional, and throttling responses are successful
health contacts even when the logical query fails. HTTP `500`, `502`, `503`, and `504` are
health-neutral: they neither reset nor advance counters and do not update health time.

#### Control-plane probes

| Probe outcome | Health action |
| --- | --- |
| Final HTTP 200 | `PROBE_SUCCESS` |
| Any other final HTTP status | `PROBE_FAILURE` |
| Failure before a final response | `PROBE_FAILURE` |

Informational responses neither complete nor classify an attempt and do not clear its in-flight
bookkeeping. Probe outcomes must not demote an active endpoint. A successful direct probe activates
quarantine; a failed quarantine probe leaves state and traffic counters unchanged. A discovery or
probe failure is not a traffic failure.

Standalone probes classify by final status without parsing response bodies. Topology discovery
requires a syntactically valid body before reporting successful contact; a valid empty array still
proves direct contact.

### State transitions

Health status exposes one `failures` value and one `successes` value. Their meanings depend on the
current state:

| State | `failures` | `successes` |
| --- | --- | --- |
| `ACTIVE` | Active traffic-failure streak. | Quarantine-promotion threshold while healthy; zero after a traffic failure. |
| `QUARANTINED` | Quarantine traffic-failure streak. | Traffic-promotion streak. |
| `DOWN` | Traffic-failure streak that triggered the most recent entry to `DOWN`, retained while down. | Recovery-probe streak. |

Entering `DOWN` retains the triggering `failures` value and resets `successes` to zero. Entering
recovery quarantine resets both values to zero. Entering `ACTIVE` resets `failures` to zero and sets
`successes` to the quarantine-promotion threshold. Implementations may maintain additional
state-specific counters internally, but their exposed health status and transitions must produce
these values.

The update time records initial status creation until an observation is accepted. Every accepted
traffic or probe observation must then set it to the time that observation is accepted, even when
state and counters do not change. Health-neutral responses and rejected stale results must not
...
| Input | Transition |
| --- | --- |
| `TRAFFIC_SUCCESS` | Remain `ACTIVE`; reset active traffic failures and store the promotion threshold as the healthy success value. |
| `TRAFFIC_FAILURE` below threshold | Remain `ACTIVE`; increment failures and reset successes. |
| `TRAFFIC_FAILURE` at threshold | Move to `DOWN`; retain failures at the triggering active threshold and reset successes to zero. |
...
| `TRAFFIC_FAILURE` at quarantine threshold | Move to `DOWN`; retain failures at the triggering quarantine threshold and reset successes to zero. |
| Health-neutral response | No state, counter, or update-time change. |
...
| Input | Transition |
| --- | --- |
| `PROBE_SUCCESS` below recovery threshold | Remain `DOWN`; retain failures and increment successes. |
| `PROBE_SUCCESS` at recovery threshold | Move to recovery quarantine with failures and successes zero. |
| `PROBE_FAILURE` | Remain `DOWN`; retain failures and reset successes to zero. |
...
failures, zero successes, and generation zero. `expected-failures` and `expected-successes` are the
state-interpreted health-status values defined above.
...
Classification uses a final HTTP status or failure before a final response, not a parsed application
exception. It must not consume, replace, or corrupt response content used by protocol parsing or
...
- An HTTP `500`, `502`, `503`, or `504` response leaves quarantine progress unchanged.
...
Every final traffic response, transport failure before a final response, and direct probe must
produce exactly the specified observation or health-neutral action.
